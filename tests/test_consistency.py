...
class TestConsistency:
    ...
    def run_consistency_sim(self, duration_s=10.0, dt=0.01):
        ...
        for i in range(N):
            ...
            dq = eskf._quat_mult(
                np.array([q_est[0], -q_est[1], -q_est[2], -q_est[3]]), true_att
            )
            dx[6:9] = 2.0 * dq[1:4]
    ...
    def test_nees_consistency(self):
        ...
        print(
            f"Mean NEES: {mean_nees:.2f} (95% CI: {lower_bound:.2f} - {upper_bound:.2f})"
        )
        ...
        if mean_nees < loose_lower:
            pytest.fail(
                f"Filter is severely UNDERCONFIDENT (Mean NEES {mean_nees:.2f} < {loose_lower:.2f})"
            )
        if mean_nees > loose_upper:
            pytest.fail(
                f"Filter is severely OVERCONFIDENT / DIVERGING (Mean NEES {mean_nees:.2f} > {loose_upper:.2f})"
            )
    ...
    def test_nis_consistency(self):
        ...
        print(
            f"Mean NIS: {mean_nis:.2f} (95% CI: {lower_bound:.2f} - {upper_bound:.2f})"
        )
        ...
        if mean_nis < loose_lower:
            pytest.fail(
                f"GPS Innovation is UNDERCONFIDENT (Mean NIS {mean_nis:.2f} < {loose_lower:.2f})"
            )
        if mean_nis > loose_upper:
            pytest.fail(
                f"GPS Innovation is OVERCONFIDENT (Mean NIS {mean_nis:.2f} > {loose_upper:.2f})"
            )
