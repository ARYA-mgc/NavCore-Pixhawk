...
from core.m import INSNavSys  # noqa: E402
...
def test_end_to_end_oosm():
    ...
    for i in range(150):
        ...
        # Mag pointing North
        msg_mag = MockMsg(
            "SCALED_IMU3", time_usec=int(t_now * 1e6), xmag=300, ymag=0, zmag=400
        )
    ...
    msg_gps = MockMsg(
        "GPS_RAW_INT",
        time_usec=int(t_gps * 1e6),
        fix_type=3,
        lat=int((13.0 + 1e-5) * 1e7),
        lon=int(80.0 * 1e7),
        alt=int(50.0 * 1000),
        eph=100,
    )
    ...
    assert post_update_cov < pre_update_cov, (
        "Covariance did not shrink after GPS update"
    )
...
def test_end_to_end_stress():
    ...
    print("\n--- OOSM Stress Test ---")
    print(
        f"Total CPU Time for 10s flight @ 400Hz with 20Hz 500ms-delayed GPS: {compute_time:.3f} s"
    )
    ...
    assert compute_time < 60.0, (
        f"OOSM replay is too slow! Took {compute_time:.3f}s for 10s of data."
    )
