...
def parse_raw_imu(msg):
    accel = np.array(
        [msg.xacc * ACCEL_SCALE, msg.yacc * ACCEL_SCALE, msg.zacc * ACCEL_SCALE]
    )
    gyro = np.array(
        [msg.xgyro * GYRO_SCALE, msg.ygyro * GYRO_SCALE, msg.zgyro * GYRO_SCALE]
    )
    return accel, gyro
...
class TestRTCMForwarding:
    ...
    def test_rtcm_reassembly_logic(self):
        ...
        reassembled = (
            frag0.data[: frag0.len] + frag1.data[: frag1.len] + frag2.data[: frag2.len]
        )
