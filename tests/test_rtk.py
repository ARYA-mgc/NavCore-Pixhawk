...
def build_nav_hpposllh_payload(
    lat_deg=13.0827,
    lon_deg=80.2707,
    alt_m=50.0,
    lat_hp=5,
...
class TestUBXNavParsing:
    ...
    def test_parse_nav_hpposllh(self):
        ...
        payload = build_nav_hpposllh_payload(
            lat_deg=lat, lon_deg=lon, alt_m=alt, lat_hp=5, lon_hp=-3, height_hp=7
        )
    ...
    def test_csv_format_matches_sim(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(10):
                t = t0 + i * 0.01
                recorder.record_imu(
                    t, np.array([0.0, 0.0, -9.81]), np.array([0.01, -0.005, 0.002])
                )
...
class TestLogValidator:
    def _create_test_logs(
        self, tmpdir, duration=10.0, imu_hz=100, rtk_hz=5, with_gaps=False
    ):
        ...
        # RTK ground truth
        with open(os.path.join(tmpdir, "rtk_ground_truth.csv"), "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(
                [
                    "time_s",
    ...
    def test_gaps_detected(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            gap_results = [r for r in report.results if "continuity" in r.name.lower()]
            if gap_results:
