...
ESKFCore::ESKFCore(const IMUNoiseParams &noise)
    : noise_(noise), health_(EKFHealth::CONVERGING), initialized_(false),
      step_count_(0), baro_bias_(0.0), baro_bias_alpha_(0.001),
      baro_update_count_(0), calibrated_mag_norm_(0.5), mag_reject_until_(0.0),
      mag_consecutive_good_(0), vibration_scale_(1.0) {
  // Initialize nominal state
  x_.setZero();
  x_(6) = 1.0; // qw = 1 (identity quaternion)

  // Initialize error-state covariance
  MatP P0 = MatP::Identity();
  P0.block<3, 3>(0, 0) *= 1.0;     // position
  ...
  Q_sqrt_ = Q_.llt().matrixU();

  // Measurement matrices
  H_baro_.setZero();
  H_baro_(0, 2) = -1.0;  // observe -pz (NED down → altitude up)
...
void ESKFCore::predict(const Vec3 &accel, const Vec3 &gyro, double dt) {
  ...
  if (angle > 1e-10) {
    ...
    double ha = angle / 2.0;
    dq << std::cos(ha), axis(0) * std::sin(ha), axis(1) * std::sin(ha),
        axis(2) * std::sin(ha);
  } else {
    dq << 1.0, 0.0, 0.0, 0.0;
  }
  ...
  // 6. State transition Jacobian and covariance propagation (QR formulation)
  MatF F = compute_F(accel_corr, gyro_corr, R, dt);
  Eigen::Matrix<double, ERROR_DIM * 2, ERROR_DIM> A;
  A.topRows<ERROR_DIM>() = U_ * F.transpose();
  A.bottomRows<ERROR_DIM>() = Q_sqrt_ * std::sqrt(dt);
  Eigen::HouseholderQR<Eigen::Matrix<double, ERROR_DIM * 2, ERROR_DIM>> qr(A);
  U_ = qr.matrixQR().topRows<ERROR_DIM>().template triangularView<Eigen::Upper>();
...
MatF ESKFCore::compute_F(const Vec3 &accel, const Vec3 &gyro, const Mat3 &R,
                         double dt) const {
...
void ESKFCore::update_baro(double alt_m) {
  ...
  // NED: x_(2) is positive-down, baro altitude is positive-up
  double z_pred = -x_(2);
  ...
  VecE a = U_ * H_baro_.transpose();
  double S = a.dot(a) + R_val;
  ...
  // Kalman gain
  VecE K = U_.transpose() * a / S;

  // Covariance downdate (Potter's)
  double gamma = 1.0 / (1.0 + std::sqrt(R_val / S));
  U_ -= gamma * a * K.transpose();
...
void ESKFCore::update_mag(double yaw_measured, double mag_norm, double t_now) {
  ...
  if (mag_norm > 0) {
    ...
    calibrated_mag_norm_ =
        (1.0 - alpha) * calibrated_mag_norm_ + alpha * mag_norm;
  }
}
...
void ESKFCore::update_optical_flow(double flow_vx, double flow_vy,
  ...
  if (!initialized_ || distance <= 0.05 || quality < 10)
    return;

  Mat3 R_dcm = quat_to_rotation(x_.segment<4>(6));

  Eigen::Matrix<double, 2, ERROR_DIM> H_flow;
  H_flow.setZero();
  // Map NED velocity error to body frame measurement
  H_flow.block<2, 3>(0, 3) = R_dcm.transpose().block<2, 3>(0, 0);

  double R_base = 0.5 * 0.5;
  Eigen::Matrix2d R_flow =
      Eigen::Matrix2d::Identity() * (R_base * 100.0 / std::max(quality, 1));

  Eigen::Vector2d z(flow_vx, flow_vy);
  
  // Predicted velocity in body frame
  Vec3 v_ned = x_.segment<3>(3);
  Vec3 v_body_pred = R_dcm.transpose() * v_ned;
  Eigen::Vector2d z_pred = v_body_pred.segment<2>(0);
...
bool ESKFCore::update_external(const VecXd &z, const VecXd &z_pred,
  ...
  // Sequential scalar updates (Potter's method)
  // Assuming R is diagonal (independent measurements)
  for (int i = 0; i < m; ++i) {
    VecE H_i = H.row(i).transpose();
    VecE a = U_ * H_i;
    double r_val = R(i, i);
    double S = a.dot(a) + r_val;
    
    double y = z(i) - z_pred(i);
    if (m == 1 && H(0, 8) != 0.0) {
      y = wrap_angle(y);
    }
    
    double nis = y * y / S;
    if (nis > CHI2_1DOF) return false;
...
bool ESKFCore::initialize_from_sensors(const Eigen::MatrixXd &accel_samples,
  ...
  double pitch =
      std::atan2(-accel_mean(0), std::sqrt(accel_mean(1) * accel_mean(1) +
...
void ESKFCore::reset() { *this = ESKFCore(noise_); }
...
void ESKFCore::inject_error(const VecE &dx) {
  ...
  assert(std::abs(x_.segment<4>(6).norm() - 1.0) < 1e-5 && "Quaternion normalization failed!");
...
void ESKFCore::harden_covariance() {
  ...
  assert((P_ - P_.transpose()).norm() < 1e-5 && "Covariance symmetry lost!");
...
void ESKFCore::check_health() {
  ...
  // Fault conditions
  if (vel_norm > VEL_FAULT || tilt > TILT_FAULT_DEG ||
      p_trace > P_TRACE_LIMIT) {
    health_ = EKFHealth::FAULT;
    return;
  }
