...
class ESKF:
    ...
    def predict(self, accel_raw: np.ndarray, gyro_raw: np.ndarray, dt: float):
        ...
        self.x[ABIAS] = np.clip(self.x[ABIAS], -self.ACCEL_BIAS_LIMIT, self.ACCEL_BIAS_LIMIT)
        self.x[GBIAS] = np.clip(self.x[GBIAS], -self.GYRO_BIAS_LIMIT, self.GYRO_BIAS_LIMIT)
        self.x[BARO_BIAS_IDX] = np.clip(self.x[BARO_BIAS_IDX], -self.BARO_BIAS_LIMIT, self.BARO_BIAS_LIMIT)
    ...
    def update_mag(self, yaw_measured: float, mag_norm: float = -1.0, t_now: float = 0.0):
        if not self._initialized:
            return
        if mag_norm > 0:
        ...
        if self.update_external(z, z_pred, self.H_mag, R_val, source="mag"):
            if mag_norm > 0:
                alpha_mag = 0.002
                self._calibrated_mag_norm = (1.0 - alpha_mag) * self._calibrated_mag_norm + alpha_mag * mag_norm
    ...
    def update_external(self, z: np.ndarray, z_pred: np.ndarray,
        ...
        if not force_accept and nis > chi2_thresh:
            ...
            if count >= 5:
                log.warning(f"RAIM FAULT: {src} rejected 5 times consecutively. Marked UNHEALTHY.")
