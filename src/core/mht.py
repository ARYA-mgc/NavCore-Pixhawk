...
class MHTManager:
    ...
    def _evaluate_and_merge(self, t_now: float):
        ...
        log.info(
            f"MHT Evaluation: Primary Cond={prim_cond:.1e}, Shadow Cond={shad_cond:.1e}"
        )
        ...
        if swap:
            log.warning(
                "MHT: Shadow filter absorbed jump successfully! Swapping to Shadow."
            )
            self.primary = self.shadow
            self.shadow = None
        elif t_now - self.shadow_spawn_time > self.MAX_SHADOW_LIFETIME:
            log.info(
                "MHT: Shadow filter failed to converge better than primary. Discarding shadow."
            )
            self.shadow = None
    ...
    def update_gps(self, lat: float, lon: float, alt: float, hdop: float, t_now: float):
        ...
        if not accepted and rejections >= 3 and self.shadow is None:
            log.warning(
                "MHT: GPS rejected 3 times consecutively. Spawning shadow filter."
            )
    ...
    def update_optical_flow(
        ...
        self.primary.update_optical_flow(
            flow_vx, flow_vy, distance, quality, enable_rot_comp=enable_rot_comp
        )
        if self.shadow:
            self.shadow.update_optical_flow(
                flow_vx, flow_vy, distance, quality, enable_rot_comp=enable_rot_comp
            )
