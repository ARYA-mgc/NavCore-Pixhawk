...
class EKF3Blender:
    ...
    def update_eskf(
        self, pos: np.ndarray, vel: np.ndarray, euler: np.ndarray, P: np.ndarray
    ):
        # Update with latest ESKF state and covariance.
        self._last_eskf_state = {  # type: ignore
    ...
    def update_ekf3(
        ...
        self._last_ekf3_state = {  # type: ignore
            "pos": np.asarray(pos, dtype=float).copy(),
            "vel": np.asarray(vel, dtype=float).copy(),
            "euler": np.asarray(euler, dtype=float).copy(),
            "P_diag": np.array(
    ...
    def _crosscheck(self, eskf: dict, ekf3: dict) -> dict:
        ...
        is_fault = (
            pos_diff > self.POS_DIVERGE_FAULT
            or vel_diff > self.VEL_DIVERGE_FAULT
            or att_diff_deg > self.ATT_DIVERGE_FAULT
        )

        is_warn = (
            pos_diff > self.POS_DIVERGE_WARN
            or vel_diff > self.VEL_DIVERGE_WARN
            or att_diff_deg > self.ATT_DIVERGE_WARN
        )
        ...
        if is_fault:
            ...
            log.error(
                f"ESKF/EKF3 DIVERGENCE FAULT #{self._diverge_fault_count}: "
    ...
    def _covariance_intersection(self, eskf: dict, ekf3: dict) -> dict:
        ...
        pos_fused, P_pos = self._ci_blend_3dof(
            eskf["pos"], eskf["P_diag"][0:3], ekf3["pos"], ekf3["P_diag"][0:3]
        )

        vel_fused, P_vel = self._ci_blend_3dof(
            eskf["vel"], eskf["P_diag"][3:6], ekf3["vel"], ekf3["P_diag"][3:6]
        )

        # Euler blending with yaw wrapping
        euler_fused = self._blend_euler(
            eskf["euler"], eskf["P_diag"][6:9], ekf3["euler"], ekf3["P_diag"][6:9]
        )
    ...
    def _ci_blend_3dof(
        ...
        for i in range(3):
            ...
            fused_p[i] = best_var
            fused_x[i] = fused_p[i] * (
                best_alpha * x1[i] / v1 + (1.0 - best_alpha) * x2[i] / v2
            )
    ...
    @staticmethod
    def _blend_euler(
        e1: np.ndarray, p1_diag: np.ndarray, e2: np.ndarray, p2_diag: np.ndarray
    ) -> np.ndarray:
        ...
        for i in range(3):
            ...
            # Wrap result
            if i == 2:
                fused[i] = math.atan2(math.sin(fused[i]), math.cos(fused[i]))
