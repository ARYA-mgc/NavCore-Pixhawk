...
# Staring at sensor noise for hours.
...
def compute_allan_variance(
    data: np.ndarray, dt: float, max_clusters: int = 100
) -> tuple:
...
def analyze_stationary_log(filepath: str, imu_rate_hz: float = 100.0) -> dict:
    ...
    for axis, data in [("x", vel_x), ("y", vel_y), ("z", vel_z)]:
        ...
        log.info(
            f"Accel {axis}: white_noise={params['white_noise']:.6f} "
    ...
    for axis, data in [("roll", euler_r), ("pitch", euler_p), ("yaw", euler_y)]:
        ...
        log.info(
            f"Gyro {axis}: white_noise={params['white_noise']:.6f} "
            f"bias_instability={params['bias_instability']:.6f}"
        )
...
def generate_tuned_yaml(
    results: dict, output_path: str = "config/noise_params_tuned.yaml"
):
    ...
    gyro_wn = np.mean(
        [results[f"gyro_{a}"]["white_noise"] for a in ["roll", "pitch", "yaw"]]
    )
    gyro_bi = np.mean(
        [results[f"gyro_{a}"]["bias_instability"] for a in ["roll", "pitch", "yaw"]]
    )
...
"""
    ...
    print(f"\n{'=' * 60}")
    print("TUNED NOISE PARAMETERS (Allan Variance)")
    print(f"{'=' * 60}")
